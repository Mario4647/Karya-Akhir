...
const AdminSiswa = () => {
  ...
  const fetchSiswaData = async () => {
    try {
      ...
      // PERBAIKAN: Menggunakan range untuk mendapatkan semua 1298 data
      // Supabase default limit adalah 1000, jadi kita perlu menggunakan range
      const { data, error, count } = await supabase
        .from('siswa')
        .select('*', { count: 'exact' })
        .order('no', { ascending: true })
        .range(0, 1299); // Mengambil data dari index 0 sampai 1299 (total 1300 data)

      if (error) throw error;

      setSiswaData(data || []);
      
      // Debug log untuk memastikan semua data terambil
      console.log(`Data siswa yang diambil: ${data ? data.length : 0}`);
      if (data && data.length > 0) {
        console.log(`No terakhir: ${data[data.length - 1].no}`);
      }
