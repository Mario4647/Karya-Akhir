...
const AdminSiswa = () => {
  ...
  const BATCH_SIZE = 100; // Load 100 data per batch
  const ITEM_HEIGHT = 60; // Tinggi setiap row
  const COLUMN_WIDTH = 120; // Lebar default kolom
  const VISIBLE_ROWS = 20; // Jumlah row yang visible sekaligus
  ...
  const filteredSiswa = useMemo(() => {
    ...
    return siswaData.filter(siswa =>
      ...
      siswa.nipd?.includes(searchTerm) ||
      siswa.rombel?.toLowerCase().includes(searchTerm.toLowerCase())
    );
  }, [siswaData, searchTerm]);

  // Virtual Grid Cell Renderer
  const Cell = useCallback(({ columnIndex, rowIndex, style }) => {
    const siswa = filteredSiswa[rowIndex];
    const column = columns[columnIndex];
    
    if (!siswa) {
      return (
        <div style={style} className="border-r border-gray-200 bg-white"></div>
      );
    }

    if (column.key === 'actions') {
      return (
        <div style={style} className="border-r border-gray-200 bg-white p-2">
    ...
    return (
      <div 
        style={style} 
        className="border-r border-gray-200 bg-white p-2 truncate"
        title={typeof displayValue === 'string' ? displayValue : undefined}
      >
        {displayValue}
      </div>
    );
  }, [filteredSiswa, columns]);
  ...
  const HeaderCell = useCallback(({ columnIndex, style }) => {
    ...
    return (
      <div 
        style={style} 
        className="border-r border-gray-300 bg-gray-50 font-semibold text-gray-700 uppercase text-xs p-2 truncate sticky top-0 z-10"
      >
        {column.header}
      </div>
    );
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6 mb-8">
            ...
            <div className="flex flex-col md:flex-row gap-4">
              <div className="flex-1 relative">
                ...
                <input
                  type="text"
                  placeholder="Cari siswa (nama, NISN, NIPD, rombel)..."
                  value={searchTerm}
                  onChange={(e) => {
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6">
            <div className="mb-4 flex justify-between items-center">
              ...
              <div className="text-sm text-gray-600">
                {hasMore && `+${totalCount - siswaData.length} data tersedia`}
              </div>
            </div>
                ...
                {({ height, width }) => (
                  <>
                    ...
                    <Grid
                      ...
                      rowCount={filteredSiswa.length}
                      rowHeight={ITEM_HEIGHT}
                      width={width}
                      onItemsRendered={({ visibleRowStopIndex }) => {
                        // Load more data when user scrolls near the end
                        if (visibleRowStopIndex > filteredSiswa.length - 10 && hasMore && !loadingMore) {
                          loadMoreData();
                        }
                      }}
                    >
            ...
            <div className="mt-4 pt-4 border-t border-gray-200">
              <div className="flex flex-col md:flex-row justify-between items-center gap-2">
                <div className="text-sm text-gray-600">
                  <span className="font-medium">Tips:</span> Gunakan scroll horizontal untuk melihat semua kolom • Scroll vertikal untuk data lebih banyak
                </div>
                <div className="text-xs text-gray-500">
                  {siswaData.length} dari {totalCount} data dimuat • {columns.length} kolom
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full transform transition-all duration-300 scale-100">
            <div className="px-6 py-4 border-b flex items-center justify-between bg-gradient-to-r from-green-50 to-emerald-50 rounded-t-2xl">
              <div>
                ...
                <p className="text-sm text-gray-600 mt-1">Support data tak terbatas • Optimized for large files</p>
              </div>
              <button
                onClick={() => {
                  setShowImportModal(false);
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <ul className="text-xs text-blue-700 space-y-1">
                        ...
                        <li>• Batch insertion untuk performa optimal</li>
                        <li>• Progress indicator selama import</li>
                        <li>• Memory efficient dengan virtual scrolling</li>
                      </ul>
                    </div>
                  </div>
                </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-6xl max-h-[90vh] overflow-hidden flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-6">
              ...
              <div className="text-center py-8 text-gray-500">
                Detail modal implementation (sama seperti sebelumnya)
              </div>
            </div>
