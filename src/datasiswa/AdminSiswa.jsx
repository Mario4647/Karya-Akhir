...
const AdminSiswa = () => {
  ...
  const fetchSiswaData = async () => {
    try {
      ...
      console.log('Memulai fetch data siswa...');
      
      // SOLUSI: Menggunakan pagination untuk mengambil semua data
      const allSiswaData = [];
      let page = 0;
      const pageSize = 1000; // Supabase maksimal per query
      ...
      while (hasMoreData) {
        ...
        if (data && data.length > 0) {
          ...
          console.log(`  Dapat ${data.length} data, total sementara: ${allSiswaData.length}`);
          
          // Jika data yang diambil kurang dari pageSize, berarti sudah habis
          if (data.length < pageSize) {
            hasMoreData = false;
            console.log(`  Data sudah habis, berhenti fetching`);
          } else {
            page++;
      ...
      // Sort by 'no' untuk memastikan urutan
      allSiswaData.sort((a, b) => (a.no || 0) - (b.no || 0));
      
      setSiswaData(allSiswaData);
      
      console.log(`Total data siswa yang berhasil diambil: ${allSiswaData.length}`);
      
      // Tampilkan detail data
      if (allSiswaData.length > 0) {
        console.log(`Data pertama: No ${allSiswaData[0].no} - ${allSiswaData[0].nama}`);
        console.log(`Data terakhir: No ${allSiswaData[allSiswaData.length - 1].no} - ${allSiswaData[allSiswaData.length - 1].nama}`);
        
        // Cek duplikat
        const uniqueNos = [...new Set(allSiswaData.map(s => s.no))];
        console.log(`Jumlah nomor unik: ${uniqueNos.length}`);
      }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-50 flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <span className="text-blue-600 text-sm">Mengambil {siswaData.length} data...</span>
        </div>
      </div>
    );
  }
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6">
            <div className="mb-4 flex justify-between items-center">
              ...
              <div className="text-sm text-gray-600">
                {filteredSiswa.length} siswa ditemukan (dari total {siswaData.length})
              </div>
            </div>
