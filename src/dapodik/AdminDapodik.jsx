...
const AdminDapodik = () => {
  ...
  const [showUploadSertifikatModal, setShowUploadSertifikatModal] = useState(false);
  const [showSertifikasiLinkModal, setShowSertifikasiLinkModal] = useState(false);
  ...
  // State untuk link sertifikasi
  const [sertifikasiLink, setSertifikasiLink] = useState('');
  
  // State untuk riwayat download
  const [downloadHistory, setDownloadHistory] = useState([]);
  const [loadingHistory, setLoadingHistory] = useState(false);
  
  // Opsi mapel pilihan
  const mapelPilihanOptions = [
  ...
  // Fungsi untuk membuka modal TKA
  const openTKAModal = (student) => {
    setSelectedStudent(student);
    
    // Load existing TKA data if available
    if (student.tka_matematika_wajib || student.nilai_tka_data) {
      if (student.nilai_tka_data && typeof student.nilai_tka_data === 'object') {
        // Jika menggunakan JSONB
        setTkaData({
          tahun_ajaran: student.nilai_tka_data.tahun_ajaran || '2024/2025',
          semester: student.nilai_tka_data.semester || 'Semester 1',
          jenis_tes: student.nilai_tka_data.jenis_tes || 'Try Out 1',
          matematika_wajib: student.nilai_tka_data.matematika_wajib || '',
          bahasa_inggris: student.nilai_tka_data.bahasa_inggris || '',
          bahasa_indonesia: student.nilai_tka_data.bahasa_indonesia || '',
          mapel_pilihan_1: student.nilai_tka_data.mapel_pilihan_1 || '',
          nilai_pilihan_1: student.nilai_tka_data.nilai_pilihan_1 || '',
          mapel_pilihan_2: student.nilai_tka_data.mapel_pilihan_2 || '',
          nilai_pilihan_2: student.nilai_tka_data.nilai_pilihan_2 || ''
        });
      } else {
        // Jika menggunakan kolom terpisah
        setTkaData({
          tahun_ajaran: student.tka_tahun_ajaran || '2024/2025',
          semester: student.tka_semester || 'Semester 1',
          jenis_tes: student.tka_jenis_tes || 'Try Out 1',
          matematika_wajib: student.tka_matematika_wajib || '',
          bahasa_inggris: student.tka_bahasa_inggris || '',
          bahasa_indonesia: student.tka_bahasa_indonesia || '',
          mapel_pilihan_1: student.tka_mapel_pilihan_1 || '',
          nilai_pilihan_1: student.tka_nilai_pilihan_1 || '',
          mapel_pilihan_2: student.tka_mapel_pilihan_2 || '',
          nilai_pilihan_2: student.tka_nilai_pilihan_2 || ''
        });
      }
    } else {
    ...
    setShowTKAModal(true);
  };

  // Fungsi untuk membuka modal upload sertifikat TKA
  const openUploadSertifikatModal = (student) => {
    setSelectedStudent(student);
    setUploadFile(null);
    setShowUploadSertifikatModal(true);
  };

  // Fungsi untuk membuka modal link sertifikasi
  const openSertifikasiLinkModal = (student) => {
    setSelectedStudent(student);
    setSertifikasiLink(student.tka_sertifikasi_link || '');
    setShowSertifikasiLinkModal(true);
  };

  // Fungsi untuk membuka modal riwayat download
  const openDownloadHistoryModal = async (student) => {
    setSelectedStudent(student);
    setLoadingHistory(true);
    try {
      const { data, error } = await supabase
        .from('tka_download_history')
        .select('*')
      ...
      setErrorMessage('Gagal memuat riwayat download');
    } finally {
      setLoadingHistory(false);
    }
  };

  // Fungsi untuk upload sertifikat TKA
  const handleUploadSertifikat = async () => {
    if (!uploadFile) {
      setErrorMessage('File sertifikat harus diisi');
      return;
    }

    if (!uploadFile.type.includes('pdf')) {
      setErrorMessage('Hanya file PDF yang diperbolehkan untuk sertifikat');
      return;
    }
    ...
    try {
      ...
      reader.onload = async (e) => {
        try {
          ...
          const { error } = await supabase
            .from('profiles')
            .update({
              tka_sertifikat_file: base64Data,
              tka_sertifikat_filename: uploadFile.name,
              tka_sertifikat_mimetype: uploadFile.type,
              tka_sertifikat_uploaded_at: new Date().toISOString(),
              tka_sertifikat_status: 'sudah_upload',
          ...
          setShowUploadSertifikatModal(false);
          setSuccessMessage('Sertifikat TKA berhasil diupload');
          fetchData();
          setTimeout(() => setSuccessMessage(''), 3000);
        } catch (error) {
          console.error('Error uploading Sertifikat PDF:', error);
          setErrorMessage('Gagal upload sertifikat: ' + error.message);
        } finally {
      ...
      console.error('Error in sertifikat upload:', error);
      setErrorMessage('Gagal upload sertifikat: ' + error.message);
      setUploadingSertifikat(false);
    }
  };

  // Fungsi untuk menyimpan link sertifikasi
  const handleSaveSertifikasiLink = async () => {
    if (!sertifikasiLink.trim()) {
      setErrorMessage('Link sertifikasi harus diisi');
      return;
    }
    ...
    try {
      // Validasi URL
      try {
        new URL(sertifikasiLink);
      } catch (error) {
        setErrorMessage('Format link tidak valid. Gunakan format: https://example.com');
        return;
      }
      ...
      const { error } = await supabase
        .from('profiles')
        .update({
          tka_sertifikasi_link: sertifikasiLink.trim(),
          tka_sertifikasi_link_updated_at: new Date().toISOString(),
          updated_at: new Date().toISOString()
        })
        .eq('id', selectedStudent.id);
      ...
      setShowSertifikasiLinkModal(false);
      setSuccessMessage('Link sertifikasi berhasil disimpan');
      fetchData();
      setTimeout(() => setSuccessMessage(''), 3000);
    } catch (error) {
      console.error('Error saving sertifikasi link:', error);
      setErrorMessage('Gagal menyimpan link sertifikasi: ' + error.message);
    }
  };

  // Fungsi untuk menghapus sertifikat TKA
  const handleDeleteSertifikat = async (studentId) => {
    try {
      const { error } = await supabase
        .from('profiles')
        .update({
          tka_sertifikat_file: null,
          tka_sertifikat_filename: null,
          tka_sertifikat_mimetype: null,
          tka_sertifikat_uploaded_at: null,
          tka_sertifikat_status: 'belum_upload',
          tka_sertifikasi_link: null,
          tka_sertifikasi_link_updated_at: null,
          updated_at: new Date().toISOString()
        })
        .eq('id', studentId);
      ...
      setSuccessMessage('Sertifikat TKA berhasil dihapus');
      fetchData();
      setTimeout(() => setSuccessMessage(''), 3000);
    } catch (error) {
      console.error('Error deleting sertifikat:', error);
      setErrorMessage('Gagal menghapus sertifikat: ' + error.message);
    }
  };

  // Fungsi untuk mendownload sertifikat TKA (untuk admin)
  const handleDownloadSertifikat = async (student) => {
    if (!student.tka_sertifikat_file) {
      setErrorMessage('Sertifikat TKA belum tersedia');
      return;
    }

    try {
      const byteCharacters = atob(student.tka_sertifikat_file);
      const byteNumbers = new Array(byteCharacters.length);
      for (let i = 0; i < byteCharacters.length; i++) {
        byteNumbers[i] = byteCharacters.charCodeAt(i);
      }
      const byteArray = new Uint8Array(byteNumbers);
      const blob = new Blob([byteArray], { type: student.tka_sertifikat_mimetype || 'application/pdf' });
      
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = student.tka_sertifikat_filename || 'sertifikat_tka.pdf';
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      window.URL.revokeObjectURL(url);
      
      setSuccessMessage('Sertifikat TKA berhasil didownload');
      setTimeout(() => setSuccessMessage(''), 3000);
    } catch (error) {
      console.error('Error downloading sertifikat:', error);
      setErrorMessage('Gagal download sertifikat: ' + error.message);
    }
  };
    ...
    try {
      const { error } = await supabase
        ...
        .update({
          ...
          // Reset data TKA juga
          nilai_tka_data: null,
          tka_matematika_wajib: null,
          tka_bahasa_inggris: null,
          tka_bahasa_indonesia: null,
          tka_mapel_pilihan_1: null,
          tka_nilai_pilihan_1: null,
          tka_mapel_pilihan_2: null,
          tka_nilai_pilihan_2: null,
          tka_tahun_ajaran: null,
          tka_semester: null,
          tka_jenis_tes: null,
          tka_total_nilai: null,
          tka_ranking: null,
          tka_diupdate_pada: null,
          // Reset sertifikat TKA
          tka_sertifikat_file: null,
          tka_sertifikat_filename: null,
          tka_sertifikat_mimetype: null,
          tka_sertifikat_uploaded_at: null,
  ...
  // Fungsi untuk menyimpan data TKA
  const handleTKASubmit = async () => {
    try {
      // Validasi data
      ...
      for (const field of requiredFields) {
        if (!tkaData[field] || isNaN(parseFloat(tkaData[field]))) {
          setErrorMessage(`Nilai ${field.replace('_', ' ')} harus diisi dengan angka`);
          return;
        }
        
        const nilai = parseFloat(tkaData[field]);
        if (nilai < 0 || nilai > 100) {
          setErrorMessage(`Nilai ${field.replace('_', ' ')} harus antara 0-100`);
          return;
        }
      }

      // Hitung total nilai
      const nilaiWajib = parseFloat(tkaData.matematika_wajib) + 
                        parseFloat(tkaData.bahasa_inggris) + 
                        parseFloat(tkaData.bahasa_indonesia);
      
      const nilaiPilihan1 = tkaData.nilai_pilihan_1 ? parseFloat(tkaData.nilai_pilihan_1) : 0;
      const nilaiPilihan2 = tkaData.nilai_pilihan_2 ? parseFloat(tkaData.nilai_pilihan_2) : 0;
      
      const totalNilai = nilaiWajib + nilaiPilihan1 + nilaiPilihan2;

      // Update data menggunakan JSONB (cara yang lebih fleksibel)
      const tkaDataToSave = {
        tahun_ajaran: tkaData.tahun_ajaran,
        semester: tkaData.semester,
        jenis_tes: tkaData.jenis_tes,
        matematika_wajib: parseFloat(tkaData.matematika_wajib),
        bahasa_inggris: parseFloat(tkaData.bahasa_inggris),
        bahasa_indonesia: parseFloat(tkaData.bahasa_indonesia),
        mapel_pilihan_1: tkaData.mapel_pilihan_1,
        nilai_pilihan_1: tkaData.nilai_pilihan_1 ? parseFloat(tkaData.nilai_pilihan_1) : null,
        mapel_pilihan_2: tkaData.mapel_pilihan_2,
        nilai_pilihan_2: tkaData.nilai_pilihan_2 ? parseFloat(tkaData.nilai_pilihan_2) : null,
      ...
      const { error } = await supabase
        .from('profiles')
        .update({
          nilai_tka_data: tkaDataToSave,
          tka_diupdate_pada: new Date().toISOString(),
          updated_at: new Date().toISOString()
        })
        .eq('id', selectedStudent.id);
      ...
      setShowTKAModal(false);
      setSuccessMessage('Data Nilai TKA berhasil disimpan');
      fetchData();
      setTimeout(() => setSuccessMessage(''), 3000);
    } catch (error) {
      console.error('Error saving TKA data:', error);
      setErrorMessage('Gagal menyimpan data TKA: ' + error.message);
    }
  };

  // Fungsi untuk menghapus data TKA
  const handleDeleteTKAData = async () => {
    try {
      const { error } = await supabase
        .from('profiles')
        .update({
      ...
      setSuccessMessage('Data Nilai TKA berhasil dihapus');
      fetchData();
      setTimeout(() => setSuccessMessage(''), 3000);
    } catch (error) {
      console.error('Error deleting TKA data:', error);
      setErrorMessage('Gagal menghapus data TKA: ' + error.message);
    }
  };
  ...
  // Fungsi untuk mendapatkan status sertifikat (hanya untuk info display)
  const getSertifikatStatus = (student) => {
    if (!student.tka_sertifikat_status || student.tka_sertifikat_status === 'belum_upload') {
      return {
        color: 'gray',
        text: 'Belum Upload',
        icon: '❌'
  ...
  // Fungsi untuk mendapatkan status download SHTK
  const getDownloadSHTKStatus = (student) => {
    // Untuk implementasi yang lebih baik, sebaiknya query ke tabel tka_download_history
    ...
    // Asumsi jika ada file sertifikat dan link, maka sudah download
    if (student.tka_sertifikat_file) {
      return {
        color: 'green',
        text: 'Sudah Download',
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-50">
                      ...
                      <tr key={student.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <div className="flex flex-wrap gap-2">
                            ...
                            <button
                              onClick={() => openTKAModal(student)}
                              className="inline-flex items-center gap-1 px-3 py-2 bg-orange-100 text-orange-700 rounded-lg hover:bg-orange-200 transition-colors text-sm"
                            >
                              📝 Nilai TKA
                            </button>
                          </div>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
              ...
              <div className="flex items-center gap-2">
                <span className="text-sm text-gray-600">
                  Total siswa dengan data TKA: {students.filter(s => s.nilai_tka_data || s.tka_matematika_wajib).length}
                </span>
              </div>
            </div>
              ...
              <table className="w-full min-w-max">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">Nama Siswa</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">Kelas</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">Matematika Wajib</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">Bahasa Inggris</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">Bahasa Indonesia</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">Mapel Pilihan 1</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">Nilai</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">Mapel Pilihan 2</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">Nilai</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">Total</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">Link Sertifikasi</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">Detail Riwayat Download</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">Status Download SHTK</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">Sertifikat TKA</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700">Aksi TKA</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
                  {students.filter(student => 
                    student.nilai_tka_data || 
                    student.tka_matematika_wajib
                  ).map((student) => {
                    let tkaData = {};
                    
                    if (student.nilai_tka_data && typeof student.nilai_tka_data === 'object') {
                      tkaData = student.nilai_tka_data;
                    } else {
                      tkaData = {
                        matematika_wajib: student.tka_matematika_wajib,
                        bahasa_inggris: student.tka_bahasa_inggris,
                        ...
                        mapel_pilihan_2: student.tka_mapel_pilihan_2,
                        nilai_pilihan_2: student.tka_nilai_pilihan_2,
                        total_nilai: student.tka_total_nilai
                      };
                    }

                    const total = tkaData.total_nilai || 
                    ...
                    const sertifikatStatus = getSertifikatStatus(student);
                    const downloadSHTKStatus = getDownloadSHTKStatus(student);
                    ...
                    return (
                      <tr key={student.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <span className={`font-medium ${tkaData.matematika_wajib ? 'text-gray-900' : 'text-gray-400'}`}>
                            {tkaData.matematika_wajib || '-'}
                          </span>
                        </td>
                        <td className="px-6 py-4">
                          <span className={`font-medium ${tkaData.bahasa_inggris ? 'text-gray-900' : 'text-gray-400'}`}>
                            {tkaData.bahasa_inggris || '-'}
                          </span>
                        </td>
                        <td className="px-6 py-4">
                          <span className={`font-medium ${tkaData.bahasa_indonesia ? 'text-gray-900' : 'text-gray-400'}`}>
                            {tkaData.bahasa_indonesia || '-'}
                          </span>
                        </td>
                        <td className="px-6 py-4">
                          <span className={`font-medium ${tkaData.mapel_pilihan_1 ? 'text-gray-900' : 'text-gray-400'}`}>
                            {tkaData.mapel_pilihan_1 || '-'}
                          </span>
                        </td>
                        <td className="px-6 py-4">
                          <span className={`font-medium ${tkaData.nilai_pilihan_1 ? 'text-gray-900' : 'text-gray-400'}`}>
                            {tkaData.nilai_pilihan_1 || '-'}
                          </span>
                        </td>
                        <td className="px-6 py-4">
                          <span className={`font-medium ${tkaData.mapel_pilihan_2 ? 'text-gray-900' : 'text-gray-400'}`}>
                            {tkaData.mapel_pilihan_2 || '-'}
                          </span>
                        </td>
                        <td className="px-6 py-4">
                          <span className={`font-medium ${tkaData.nilai_pilihan_2 ? 'text-gray-900' : 'text-gray-400'}`}>
                        ...
                        {/* Kolom Link Sertifikasi */}
                        <td className="px-6 py-4">
                          <div className="flex flex-col gap-2">
                            <button
                              onClick={() => openSertifikasiLinkModal(student)}
                              className="inline-flex items-center gap-1 px-3 py-2 bg-indigo-100 text-indigo-700 rounded-lg hover:bg-indigo-200 transition-colors text-sm"
                            >
                              🔗 Masukan Link Sertifikasi
                            </button>
                            {student.tka_sertifikasi_link && (
                              <div className="text-xs">
                                <p className="text-green-600">✓ Link tersedia</p>
                                <a 
                                  href={student.tka_sertifikasi_link} 
                                  target="_blank" 
                                  rel="noopener noreferrer"
                                  className="text-blue-600 hover:underline truncate block"
                                  style={{ maxWidth: '150px' }}
                                >
                                  {student.tka_sertifikasi_link.substring(0, 30)}...
                                </a>
                              </div>
                            )}
                          </div>
                        ...
                        {/* Kolom Detail Riwayat Download */}
                        <td className="px-6 py-4">
                          <button
                            onClick={() => openDownloadHistoryModal(student)}
                        ...
                        {/* Kolom Status Download SHTK */}
                        <td className="px-6 py-4">
                          <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-${downloadSHTKStatus.color}-100 text-${downloadSHTKStatus.color}-800`}>
                            <span className="mr-1">{downloadSHTKStatus.icon}</span>
                            {downloadSHTKStatus.text}
                          </span>
                        </td>
                        
                        {/* Kolom Sertifikat TKA */}
                        <td className="px-6 py-4">
                          <div className="flex flex-col gap-2">
                            {/* Tombol Upload Sertifikat */}
                            <div className="flex gap-2">
                              <button
                                onClick={() => openUploadSertifikatModal(student)}
                                className="inline-flex items-center gap-1 px-3 py-2 bg-teal-100 text-teal-700 rounded-lg hover:bg-teal-200 transition-colors text-sm"
                              >
                                📤 Upload Sertifikat
                              </button>
                              
                              {student.tka_sertifikat_file && (
                                <>
                                  <button
                                    onClick={() => handleDownloadSertifikat(student)}
                                  ...
                                  <button
                                    onClick={() => {
                                      if (window.confirm(`Hapus sertifikat TKA untuk ${student.name}?`)) {
                                        handleDeleteSertifikat(student.id);
                                      }
                                    }}
                            ...
                            <div className="text-xs">
                              {student.tka_sertifikat_file ? (
                                <div className="text-green-600">
                                  ...
                                  {student.tka_sertifikat_uploaded_at && (
                                    <div className="text-gray-500 text-xs">
                                      Diupload: {formatDateTime(student.tka_sertifikat_uploaded_at)}
                                    </div>
                                  )}
                                </div>
                              ) : (
                        ...
                        <td className="px-6 py-4">
                          <div className="flex flex-wrap gap-2">
                            ...
                            <button
                              onClick={() => {
                                if (window.confirm(`Apakah Anda yakin ingin menghapus data TKA untuk ${student.name}?`)) {
                                  setSelectedStudent(student);
                                  handleDeleteTKAData();
                                }
                              }}
                  ...
                  {students.filter(s => s.nilai_tka_data || s.tka_matematika_wajib).length === 0 && (
                    <tr>
                      <td colSpan="16" className="px-6 py-8 text-center text-gray-500">
                        <div className="flex flex-col items-center gap-2">
                          <span className="text-4xl text-gray-300">📊</span>
                          <p>Belum ada data Nilai TKA</p>
                          <p className="text-sm">Klik tombol "Nilai TKA" pada tabel siswa untuk menambahkan data</p>
                        </div>
                      </td>
                    </tr>
                  )}
      ...
      {showTKAModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b px-6 py-4 flex items-center justify-between">
              <h3 className="text-xl font-bold text-gray-800">Input Nilai TKA</h3>
              <button
                onClick={() => setShowTKAModal(false)}
                className="text-gray-500 hover:text-gray-700"
              >
                ✕
              </button>
            ...
            <div className="p-6">
              <div className="mb-6 p-4 bg-blue-50 rounded-lg">
                <p className="text-sm text-blue-800">
                  <strong>Siswa:</strong> {selectedStudent?.name} | <strong>Kelas:</strong> {selectedStudent?.class} | <strong>NISN:</strong> {selectedStudent?.nisn || '-'}
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <div>
                  ...
                  <select
                    value={tkaData.tahun_ajaran}
                    onChange={(e) => setTkaData({...tkaData, tahun_ajaran: e.target.value})}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                  >
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setTkaData({...tkaData, semester: e.target.value})}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                  >
                ...
                <div>
                  ...
                  <select
                    value={tkaData.jenis_tes}
                    onChange={(e) => setTkaData({...tkaData, jenis_tes: e.target.value})}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                  >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <div>
                  ...
                  <input
                    ...
                    max="100"
                    step="0.1"
                    value={tkaData.matematika_wajib}
                    onChange={(e) => setTkaData({...tkaData, matematika_wajib: e.target.value})}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                ...
                <div>
                  ...
                  <input
                    ...
                    max="100"
                    step="0.1"
                    value={tkaData.bahasa_inggris}
                    onChange={(e) => setTkaData({...tkaData, bahasa_inggris: e.target.value})}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                    placeholder="Contoh: 78.0"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    max="100"
                    step="0.1"
                    value={tkaData.bahasa_indonesia}
                    onChange={(e) => setTkaData({...tkaData, bahasa_indonesia: e.target.value})}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
                <div className="space-y-4">
                  <div>
                    ...
                    <select
                      value={tkaData.mapel_pilihan_1}
                      onChange={(e) => setTkaData({...tkaData, mapel_pilihan_1: e.target.value})}
                      className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                    >
                      <option value="">Pilih Mapel</option>
                      {mapelPilihanOptions.map((mapel) => (
                  ...
                  <div>
                    ...
                    <input
                      ...
                      max="100"
                      step="0.1"
                      value={tkaData.nilai_pilihan_1}
                      onChange={(e) => setTkaData({...tkaData, nilai_pilihan_1: e.target.value})}
                      className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <select
                      value={tkaData.mapel_pilihan_2}
                      onChange={(e) => setTkaData({...tkaData, mapel_pilihan_2: e.target.value})}
                      className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                    >
                      <option value="">Pilih Mapel</option>
                      {mapelPilihanOptions.map((mapel) => (
                  ...
                  <div>
                    ...
                    <input
                      ...
                      max="100"
                      step="0.1"
                      value={tkaData.nilai_pilihan_2}
                      onChange={(e) => setTkaData({...tkaData, nilai_pilihan_2: e.target.value})}
                      className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                      placeholder="Kosongkan jika tidak ada"
                    />
                  </div>
              ...
              <div className="mt-6 p-4 bg-gray-50 rounded-lg">
                <h5 className="font-medium text-gray-800 mb-2">Preview Total Nilai:</h5>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  <div className="text-center p-3 bg-white rounded border">
                    <div className="text-sm text-gray-600">Matematika Wajib</div>
                    <div className="text-lg font-bold">{tkaData.matematika_wajib || 0}</div>
                  </div>
                  <div className="text-center p-3 bg-white rounded border">
                    <div className="text-sm text-gray-600">Bahasa Inggris</div>
                    <div className="text-lg font-bold">{tkaData.bahasa_inggris || 0}</div>
                  </div>
                  <div className="text-center p-3 bg-white rounded border">
                    <div className="text-sm text-gray-600">Bahasa Indonesia</div>
                    <div className="text-lg font-bold">{tkaData.bahasa_indonesia || 0}</div>
                  </div>
                  <div className="text-center p-3 bg-white rounded border">
                    <div className="text-sm text-gray-600">Total Sementara</div>
            ...
            <div className="sticky bottom-0 bg-white border-t px-6 py-4 flex justify-between gap-3">
              <div>
                <button
                  onClick={() => {
                    if (window.confirm('Hapus data TKA untuk siswa ini?')) {
                      handleDeleteTKAData();
                      setShowTKAModal(false);
                    }
                  }}
      ...
      {/* Upload Sertifikat TKA Modal */}
      {showUploadSertifikatModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full">
            <div className="px-6 py-4 border-b flex items-center justify-between">
              <h3 className="text-xl font-bold text-gray-800">Upload Sertifikat TKA</h3>
              <button
                onClick={() => setShowUploadSertifikatModal(false)}
                className="text-gray-500 hover:text-gray-700"
              >
                ✕
              </button>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center">
                    ...
                    />
                    <label htmlFor="sertifikat-upload" className="cursor-pointer">
                ...
                <div className="p-4 bg-blue-50 rounded-lg border border-blue-100">
                  <h4 className="text-sm font-medium text-blue-800 mb-2">Informasi Admin</h4>
                  <p className="text-xs text-blue-700">
                    Upload sertifikat TKA dalam format PDF. Setelah diupload, siswa akan dapat mendownload sertifikat mereka.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="px-6 py-4 border-t flex justify-end gap-3">
              ...
              <button
                ...
                disabled={uploadingSertifikat || !uploadFile}
                className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg transition-colors ${
                  uploadingSertifikat || !uploadFile
      ...
      {/* Sertifikasi Link Modal */}
      {showSertifikasiLinkModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full">
            <div className="px-6 py-4 border-b flex items-center justify-between">
              <h3 className="text-xl font-bold text-gray-800">Link Sertifikasi SHTKA</h3>
              <button
                onClick={() => setShowSertifikasiLinkModal(false)}
                className="text-gray-500 hover:text-gray-700"
              >
                ✕
              </button>
            ...
            <div className="p-6">
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Link Sertifikasi SHTKA
                  </label>
                  <input
                    type="url"
                    value={sertifikasiLink}
                    onChange={(e) => setSertifikasiLink(e.target.value)}
                    placeholder="https://example.com/sertifikasi"
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                  />
                  <p className="text-xs text-gray-500 mt-1">
                    Masukkan link valid untuk verifikasi sertifikasi SHTKA
                  </p>
                </div>
                ...
                <div className="p-4 bg-blue-50 rounded-lg border border-blue-100">
                  ...
                  <p className="text-xs text-blue-700">
                    Link ini akan digunakan oleh siswa untuk verifikasi sertifikasi SHTKA mereka.
                    Pastikan link valid dan dapat diakses oleh siswa.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="px-6 py-4 border-t flex justify-end gap-3">
              ...
              <button
                onClick={handleSaveSertifikasiLink}
                disabled={!sertifikasiLink.trim()}
                className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg transition-colors ${
                  !sertifikasiLink.trim()
                    ? 'bg-gray-400 text-white cursor-not-allowed'
                    : 'bg-indigo-600 text-white hover:bg-indigo-700'
                }`}
              >
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[80vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b px-6 py-4 flex items-center justify-between">
              <h3 className="text-xl font-bold text-gray-800">
                Riwayat Download Sertifikat - {selectedStudent?.name}
              </h3>
              <button
                onClick={() => setShowDownloadHistoryModal(false)}
                className="text-gray-500 hover:text-gray-700"
              >
                ✕
              </button>
            ...
            <div className="p-6">
              {loadingHistory ? (
                ...
                <div className="text-center py-8">
                  <span className="text-4xl text-gray-300 mb-3 block">📊</span>
                  <p className="text-gray-600">Belum ada riwayat download</p>
                </div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-sm text-left text-gray-500">
                    <thead className="text-xs text-gray-700 uppercase bg-gray-50">
                      <tr>
                        ...
                        <th className="px-6 py-3">Jam Download</th>
                        <th className="px-6 py-3">Status Download</th>
                      </tr>
                    </thead>
                    <tbody>
                      {downloadHistory.map((history, index) => {
                        const downloadDate = new Date(history.downloaded_at);
                        const hari = downloadDate.toLocaleDateString('id-ID', { weekday: 'long' });
                        const tanggal = formatDate(history.downloaded_at);
                        const jam = downloadDate.toLocaleTimeString('id-ID', {
                          hour: '2-digit',
                          minute: '2-digit',
                          second: '2-digit'
                        });
                        ...
                        return (
                          <tr key={history.id} className="bg-white border-b hover:bg-gray-50">
                            ...
                            <td className="px-6 py-4">{history.user_class || '-'}</td>
                            <td className="px-6 py-4">{history.user_nisn || '-'}</td>
                            <td className="px-6 py-4 font-mono">{history.ip_address || '-'}</td>
                            <td className="px-6 py-4">
                              <div className="flex flex-col">
                                <span>{hari}</span>
