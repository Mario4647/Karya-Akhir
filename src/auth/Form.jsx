...
const Form = () => {
    ...
    const handleSignUpChange = (e) => {
        ...
        setSignUpData(prev => ({ ...prev, [name]: sanitizedValue }));
        if (errors[name]) {
    ...
    const handleSignInChange = (e) => {
        ...
        // Sanitasi input untuk keamanan
        const sanitizedValue = name === 'email' ? value : sanitizeInput(value);
        setSignInData(prev => ({ ...prev, [name]: sanitizedValue }));
        if (errors[name]) {
            setErrors(prev => ({ ...prev, [name]: "" }));
        }
        if (submitSuccess) {
    ...
    const handleSignUp = async (e) => {
        ...
        // Rate limiting
        const rateCheck = globalRateLimiter.check(signUpData.email, 'signup');
    ...
    const handleSignIn = async (e) => {
        ...
        // Rate limiting
        const rateCheck = globalRateLimiter.check(signInData.email, 'login');
        if (!rateCheck.allowed) {
            setErrors({ submit: rateCheck.reason });
            return;
        }
        ...
        try {
            ...
            if (authError) {
                ...
                globalRateLimiter.recordFailure(signInData.email);
