...
const DapodikDashboard = () => {
  ...
  // State baru untuk fitur sertifikasi SHTKA
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      if (floatingButtonRef.current && showFloatingButton) {
  ...
  const checkAccessAndFetchData = async () => {
    try {
      ...
      setHasSentMessage(profile.has_sent_admin_message || false);
      
      // Cek apakah user bisa mengirim pesan ke admin
      await checkMessageAccess(profile);
      
      // Ambil data admin untuk ditampilkan di modal
  ...
  // Fungsi untuk mengecek akses pesan
  const checkMessageAccess = async (profile) => {
    try {
      // Cek apakah user sudah mengirim pesan sebelumnya
      if (profile.has_sent_admin_message) {
        setShowMessageModal(false);
      ...
      if (profile.can_send_admin_message) {
        // Tampilkan modal jika belum pernah mengirim
        setTimeout(() => {
  ...
  const handleSendMessage = async (sendNow = true) => {
    if (!message.trim()) {
    ...
    if (!adminData) {
      setErrorMessage('Data admin tidak ditemukan');
      return;
    }
    ...
    try {
      if (sendNow) {
        ...
        setSuccessMessage('Pesan berhasil dikirim ke admin');
        setShowMessageModal(false);
        setShowFloatingButton(false);
        setHasSentMessage(true);
      } else {
        // Simpan untuk dikirim nanti (tampilkan tombol floating)
        setShowMessageModal(false);
        setShowFloatingButton(true);
        setSuccessMessage('Anda dapat mengirim pesan nanti dengan mengklik tombol di pojok kanan bawah');
      }
  ...
  // Fungsi untuk mengirim pesan dari tombol floating
  const handleFloatingButtonClick = () => {
    setShowMessageModal(true);
  };

  // Fungsi untuk mengirim pesan yang ditunda
  const handleSendDelayedMessage = async () => {
    await handleSendMessage(true);
  };

  // Fungsi untuk mengambil riwayat download sertifikat
  ...
  // Fungsi untuk mencatat download sertifikat TKA
  const recordDownloadHistory = async () => {
    try {
      // Dapatkan IP Address pengguna (sederhana)
  ...
  const handleDownloadSertifikatTKA = async () => {
    if (!userData.tka_sertifikat_file) {
    ...
    try {
      const byteCharacters = atob(userData.tka_sertifikat_file);
      const byteNumbers = new Array(byteCharacters.length);
      for (let i = 0; i < byteCharacters.length; i++) {
        byteNumbers[i] = byteCharacters.charCodeAt(i);
      }
      const byteArray = new Uint8Array(byteNumbers);
      const blob = new Blob([byteArray], { type: userData.tka_sertifikat_mimetype || 'application/pdf' });
      
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = userData.tka_sertifikat_filename || 'sertifikat_tka.pdf';
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
  ...
  // Fungsi untuk membuka link sertifikasi
  const handleOpenSertifikasiLink = () => {
    if (userData.tka_sertifikasi_link) {
      window.open(userData.tka_sertifikasi_link, '_blank');
    } else {
      setErrorMessage('Link sertifikasi belum tersedia');
    }
  };
  ...
  const calculateTKARataRata = (tkaData) => {
    if (!tkaData) return 0;
    
    const total = calculateTKATotal(tkaData);
    const jumlahMapel = 
      (tkaData.matematika_wajib ? 1 : 0) +
      (tkaData.bahasa_inggris ? 1 : 0) +
      (tkaData.bahasa_indonesia ? 1 : 0) +
      (tkaData.nilai_pilihan_1 ? 1 : 0) +
      (tkaData.nilai_pilihan_2 ? 1 : 0);
    
    return jumlahMapel > 0 ? total / jumlahMapel : 0;
  };

  // Fungsi untuk mendapatkan status sertifikat
  const getSertifikatStatus = () => {
    if (!userData.tka_sertifikat_status || userData.tka_sertifikat_status === 'belum_upload') {
      return {
        color: 'gray',
        text: 'Sertifikat Belum Tersedia',
        icon: '❌',
        disabled: true
      };
    }
  ...
  const tkaRataRata = tkaData ? calculateTKARataRata(tkaData) : 0;
  const nilaiWajib = tkaData ? 
    (parseFloat(tkaData.matematika_wajib) || 0) + 
    (parseFloat(tkaData.bahasa_inggris) || 0) + 
    (parseFloat(tkaData.bahasa_indonesia) || 0) : 0;
  const nilaiPilihan = tkaData ? 
    (parseFloat(tkaData.nilai_pilihan_1) || 0) + 
    (parseFloat(tkaData.nilai_pilihan_2) || 0) : 0;
  
  const sertifikatStatus = getSertifikatStatus();
  ...
  return (
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="flex flex-col md:flex-row items-start gap-6">
            ...
            <div className="flex-1">
              ...
              <div className="flex flex-wrap gap-4 mt-4">
                ...
                {userData.tka_sertifikat_file && (
                  <div className="bg-teal-50 rounded-lg p-3">
                    <div className="text-sm text-gray-600 mb-1">Sertifikat</div>
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            ...
            <div className="mb-6 p-4 bg-gray-50 rounded-lg">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <p className="font-medium text-gray-900">{tkaData.tahun_ajaran || '-'}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600 mb-1">Semester</p>
                  <p className="font-medium text-gray-900">{tkaData.semester || '-'}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600 mb-1">Jenis Tes</p>
                  <p className="font-medium text-gray-900">{tkaData.jenis_tes || '-'}</p>
                </div>
              </div>
            </div>
            ...
            <div className="overflow-x-auto mb-6">
              <table className="w-full text-sm border border-gray-300">
                <thead className="text-xs text-gray-700 uppercase bg-gray-50">
                  <tr>
                    <th className="px-4 py-3 border">MAPEL WAJIB</th>
                    <th className="px-4 py-3 border">NILAI</th>
                    <th className="px-4 py-3 border">MAPEL PILIHAN 1</th>
                    <th className="px-4 py-3 border">NILAI</th>
                    <th className="px-4 py-3 border">MAPEL PILIHAN 2</th>
                    <th className="px-4 py-3 border">NILAI</th>
                    <th className="px-4 py-3 border">TOTAL</th>
                  </tr>
                </thead>
                <tbody>
                  {/* Baris 1: Matematika Wajib dan Mapel Pilihan */}
                  <tr className="bg-white hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 border text-center">
                      <span className="text-lg font-bold text-red-600">
                        {(() => {
                          const totalMatematika = parseFloat(tkaData.matematika_wajib) || 0;
                          const totalPilihan1 = parseFloat(tkaData.nilai_pilihan_1) || 0;
                          const totalPilihan2 = parseFloat(tkaData.nilai_pilihan_2) || 0;
                          return (totalMatematika + totalPilihan1 + totalPilihan2).toFixed(2);
                        })()}
                      </span>
                    </td>
                  </tr>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
              ...
              <div className="bg-gray-50 rounded-lg p-4 text-center">
                ...
                <div className="text-2xl font-bold text-gray-700">
                  {tkaRataRata.toFixed(2)}
                </div>
              </div>
            </div>

            {/* Sertifikat TKA Section */}
            <div className="mt-8 p-6 bg-teal-50 rounded-xl border border-teal-100">
              <div className="flex flex-col md:flex-row md:items-center justify-between mb-4">
                ...
                <div className="mt-4 md:mt-0 flex flex-wrap gap-2">
                  <button
                    onClick={handleDownloadSertifikatTKA}
                    disabled={sertifikatStatus.disabled || downloadingSertifikat}
                    className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg transition-colors ${
                      sertifikatStatus.disabled || downloadingSertifikat
                  ...
                  >
                    {downloadingSertifikat ? (
                      ...
                      <>
                        <i className="bx bx-download"></i>
                        {sertifikatStatus.text}
                      </>
                    )}
                  </button>
                  
                  {/* Tombol Cek Sertifikasi SHTKA - Tampil setelah download */}
                  {userData.tka_sertifikat_file && userData.tka_sertifikasi_link && (
                    <button
                      onClick={handleOpenSertifikasiLink}
                      className="inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg transition-colors"
                    >
                      <i className="bx bx-link-external"></i>
                      Cek Sertifikasi SHTKA
                    </button>
                  )}
                  
                  {/* Tombol Riwayat Download - Tampil jika ada riwayat */}
              ...
              {/* Keterangan Status Sertifikat */}
              <div className="mt-4">
                {userData.tka_sertifikat_status === 'belum_upload' ? (
                  ...
                  <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        <p className="text-sm text-green-800 font-medium">Sertifikat Tersedia</p>
                        <p className="text-xs text-green-700 mt-1">
                          Anda dapat mendownload sertifikat TKA resmi. File akan didownload dalam format PDF.
                        </p>
                        {userData.tka_sertifikat_uploaded_at && (
                          <p className="text-xs text-gray-600 mt-2">
                            Diupload: {formatDateTime(userData.tka_sertifikat_uploaded_at)}
                          </p>
                        )}
                        {userData.tka_sertifikat_filename && (
                          <p className="text-xs text-gray-600">
                            Nama file: {userData.tka_sertifikat_filename}
                          </p>
                        )}
                        {userData.tka_sertifikasi_link && (
                          <p className="text-xs text-indigo-600 mt-2">
                            <i className="bx bx-link"></i> Link sertifikasi tersedia
                          </p>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="p-3 bg-gray-50 border border-gray-200 rounded-lg">
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        <p className="text-sm text-gray-800 font-medium">Menunggu Upload</p>
                        <p className="text-xs text-gray-700 mt-1">
                          Admin sedang memproses upload sertifikat TKA.
                        </p>
                      </div>
                    </div>
                  </div>
            ...
            <div className="mt-6 p-4 bg-red-50 rounded-xl border border-red-100">
              ...
              <p className="text-sm text-red-700 mb-2">
                ...
                yang diujikan dalam tes TKA. Data ini hanya bisa diinput dan diedit oleh admin.
              </p>
              {tkaData.diupdate_pada && (
                <p className="text-xs text-gray-600">
                  Terakhir diperbarui: {formatDateTime(tkaData.diupdate_pada)}
                </p>
              )}
            </div>
          </div>
            ...
            <div className="space-y-6">
              ...
              {tkaData && (
                <div className="border-t pt-6 mt-6">
                  ...
                  {/* Info Sertifikat TKA di Detail Modal */}
                  {userData.tka_sertifikat_file && (
                    <div className="mt-6 pt-6 border-t">
                      <h5 className="text-md font-semibold text-gray-800 mb-3">Sertifikat TKA</h5>
                      <div className="p-4 bg-teal-50 rounded-lg">
                        <div className="flex items-center justify-between">
                          <div>
                            <p className="text-sm text-teal-800 font-medium">Sertifikat Tersedia</p>
                            <p className="text-xs text-teal-700 mt-1">
                              {userData.tka_sertifikat_filename || 'sertifikat_tka.pdf'}
                            </p>
                            {userData.tka_sertifikat_uploaded_at && (
      ...
      {/* Modal Pesan untuk Admin */}
      {showMessageModal && adminData && !hasSentMessage && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div ref={messageModalRef} className="bg-white rounded-2xl shadow-2xl max-w-md w-full">
            <div className="px-6 py-4 border-b flex items-center justify-between">
              <h3 className="text-xl font-bold text-gray-800">Pesan untuk Admin</h3>
              <button
            ...
            <div className="p-6">
              <div className="mb-6">
                <div className="bg-blue-50 rounded-lg p-4 mb-4">
                  <h4 className="font-medium text-blue-800 mb-2">Informasi Admin</h4>
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      <span className="text-blue-600">👤</span>
                      <span className="text-blue-700">Nama: {adminData.name}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-blue-600">📧</span>
                      <span className="text-blue-700">Email: {adminData.email}</span>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  />
                  <p className="text-xs text-gray-500 mt-1">
                    Anda dapat mengirim pesan sekarang atau nanti
                  </p>
                </div>
              </div>
            </div>
      ...
      {showDownloadHistory && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-lg max-w-4xl w-full max-h-[80vh] overflow-y-auto p-6">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-semibold text-gray-800">Riwayat Download Sertifikat TKA</h3>
              <button
                ...
                <table className="w-full text-sm text-left text-gray-500">
                  <thead className="text-xs text-gray-700 uppercase bg-gray-50">
                    <tr>
                      ...
                      <th className="px-6 py-3">Waktu Download</th>
                  ...
                  <tbody>
                    {downloadHistory.map((history, index) => (
                      <tr key={history.id} className="bg-white border-b hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4">{formatDateTimeFull(history.downloaded_at)}</td>
                        <td className="px-6 py-4">
      ...
      {/* Tombol Floating untuk Kirim Pesan */}
      {showFloatingButton && !hasSentMessage && (
        <button
        ...
        >
          ...
          <span className="ml-2 font-medium">Kirim Pesan</span>
        </button>
      )}
    </div>
  );
